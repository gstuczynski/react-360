 ...
 * 
 */
import { Controls, DeviceOrientationCameraController, MousePanCameraController, MouseRaycaster, ScrollPanCameraController, TouchRaycaster } from 'react-360-controls';
...
const intersect = [0, 0, 0]; // Events to trigger the click event, will eventually need to be replaced with
// onInput / VrButton

const CLICK_EVENTS = ['click', 'touchstart']; // Events that count as user interaction for the initialization of media
...
export default class Container {
  constructor(options = {}) {
        ...
        for (const s in this._surfaces) {
          ...
          if (surface.computeIntersection(intersect, ray.origin, ray.direction)) {
            ...
            if (showCursor) {
              ...
              const cursorMag = Math.sqrt(cursorPosition[0] * cursorPosition[0] + cursorPosition[1] * cursorPosition[1] + cursorPosition[2] * cursorPosition[2]);
    ...
    _defineProperty(this, "enterVR", () => {
      ...
      display.requestPresent([{
        ...
        this.resize(leftParams.renderWidth + rightParams.renderWidth, Math.min(leftParams.renderHeight, rightParams.renderHeight), 1);
    ...
    const _gl = this._canvas.getContext('webgl', {
      xrCompatible: true,
      alpha: false
    });
    ...
    this._gl = _gl;
    this._sharedTextureManager = new GLUI.TextureManager(_gl);
    this.group = new WebGL.RenderGroup(_gl);
    this.environment = new Environment(_gl, this.group);
  ...
  createSurface(name, width, height) {
  ...
  resize(w, h, dpr = window.devicePixelRatio) {
    this._width = w;
    this._height = h;
    ...
    const near = 0.1;
    const far = 1000;
    const f = 1 / Math.tan(fov / 2); // prettier-ignore

    this.group.setUniform('projectionMatrix', [f / aspect, 0, 0, 0, 0, f, 0, 0, 0, 0, (near + far) / (near - far), -1, 0, 0, near * far * 2 / (near - far), 0]);
  }
  /**
