...
export default class Geometry {
  ...
  addAttribute(attr, options = {}) {
    ...
    const {
      size,
      type
    } = getAttributeSizeAndType(gl, attr, normalize);
  ...
  bindToAttributes() {
    ...
    const gl = this._gl; // If using a WebGL2 context, construct a VAO the first time attributes are bound

    if ('WebGL2RenderingContext' in window && gl instanceof window.WebGL2RenderingContext) {
      this._vertexArray = gl.createVertexArray(); // $FlowFixMe - Flow lib does not include WebGL2
    ...
    for (let i = 0; i < this._attributes.length; i++) {
      ...
      this._buffer.bindToAttribute(attr.loc, attr.size, attr.type, !!attr.normalize, this._interleaved ? this._totalByteLength : attr.stride, attr.offset);
    }
  ...
  draw() {
    if (this._interleaved && this._dataLength % this._totalByteLength !== 0) {
      console.warn('Geometry buffer length is not perfectly divisible by stride. This can cause unintended errors');
    }
