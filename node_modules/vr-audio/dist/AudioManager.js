...
export default class AudioManager {
  ...
  load(source) {
    ...
    const loader = fetch(uri).then(response => response.arrayBuffer()).then(compressed => new Promise(resolve => this._awaitAudioCtx.then(ctx => {
      resolve([ctx, compressed]);
    }))).then(([ctx, compressed]) => decodeAudioData(ctx, compressed)).then(buffer => {
  ...
  setViewParameters(position, rotation) {
