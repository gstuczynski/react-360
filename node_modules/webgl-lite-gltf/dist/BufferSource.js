...
export function extractBufferSources(buffers) {
  const sources = [];
    ...
    if (uri.startsWith(GLTF_BUFFER_PREFIX) || uri.startsWith(OCTET_STREAM_PREFIX)) {
      ...
      if (typeof byteLength === 'number') {
        if (byteLength !== data.byteLength) {
          throw new Error(`Error parsing buffer. Expected ${byteLength} bytes, got ${data.byteLength} instead.`);
        }
      }
...
export function getBufferData(source) {
  switch (source.type) {
    case 'local':
