...
const getGamepads = (navigator.getGamepads ? navigator.getGamepads : navigator.webkitGetGamepads ? navigator.webkitGetGamepads : () => []).bind(navigator);
export default class VRInputSource {
  constructor() {
    ...
    window.addEventListener('gamepadconnected', e => {
      ...
      this._fireCallbacks(SourceEventTypes.INPUT_SOURCES_CHANGE, {
  ...
  setActiveSource(index) {
    if (index < 0) {
      if (this._activeSource) {
        ...
        this._fireCallbacks(SourceEventTypes.ACTIVE_SOURCE_CHANGE, {
          gamepadIndex: -1
        });
      }
    }
