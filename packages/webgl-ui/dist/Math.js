 ...
 */
// prettier-ignore
export function rotateByQuaternion(v, q) {
...
export function quaternionPremultiply(b, a) {
  const ax = a[0];
  const ay = a[1];
  const az = a[2];
  const aw = a[3];
...
export function setQuatFromEuler(quat, x, y, z) {
...
export function setQuatFromAxisAngle(quat, axis, angle) {
  const half = angle / 2;
...
export function setQuatFromXRotation(quat, x) {
  const half = x / 2;
...
export function setQuatFromYRotation(quat, y) {
  const half = y / 2;
  quat[0] = 0;
...
export function setQuatFromZRotation(quat, z) {
  const half = z / 2;
  quat[0] = 0;
  quat[1] = 0;
...
export function matrixMultiply4(a, b) {
